...
const inquiry = computed(() => localePath({ name: 'kontakt', query: { objekt: props.listing.slug } }))
</script>

<template>
  <aside class="rounded-2xl bg-secondary p-6" :aria-label="t('common.contactPersonAria')">
    <div class="flex items-center gap-4">
      ...
      <div>
        ...
        <p class="text-sm text-muted-foreground">{{ t('common.contactPerson') }}</p>
      </div>
    </div>
    <div class="mt-6 grid gap-2">
      <Button as-child size="cta">
        <NuxtLink :to="inquiry"><Mail class="size-4" aria-hidden="true" />{{ t('common.inquire') }}</NuxtLink>
      </Button>
