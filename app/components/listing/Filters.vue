...
const typOptions = [{ value: 'alle', label: 'Alle' }, { value: 'kauf', label: 'Kaufen' }, { value: 'miete', label: 'Mieten' }]
...
<template>
  <div>
    ...
    <div class="hidden flex-wrap items-center gap-2 lg:flex">
      <SiteSegmented v-model="typModel" :options="typOptions" label="Vermarktungsart" />
      <Select v-model="artModel">
        <SelectTrigger size="cta" aria-label="Objektart"><SelectValue /></SelectTrigger>
        <SelectContent>
      ...
      <Select v-model="zimmerModel">
        <SelectTrigger size="cta" aria-label="Zimmer"><SelectValue /></SelectTrigger>
        <SelectContent>
      ...
      <Select v-model="preisModel">
        <SelectTrigger size="cta" aria-label="Preis bis"><SelectValue /></SelectTrigger>
        <SelectContent>
      ...
      <Button v-if="active" variant="ghost" size="cta" @click="emit('reset')">Filter zurücksetzen</Button>
      <div class="ml-auto">
        <Select v-model="sortModel">
          <SelectTrigger size="cta" aria-label="Sortierung"><SelectValue /></SelectTrigger>
          <SelectContent>
      ...
      <Sheet v-model:open="open">
        ...
        <SheetContent side="bottom" class="max-h-[85dvh] overflow-y-auto rounded-t-3xl">
          ...
          <div class="grid gap-5 px-4 pb-6">
            <SiteSegmented v-model="typModel" :options="typOptions" label="Vermarktungsart" full />
            ...
            <div class="flex gap-3 pt-2">
              <Button variant="ghost" size="cta" class="flex-1" @click="emit('reset')">Filter zurücksetzen</Button>
              <Button size="cta" class="flex-1" @click="open = false">{{ count }} {{ count === 1 ? 'Angebot' : 'Angebote' }} ansehen</Button>
            </div>
          </div>
        </SheetContent>
      </Sheet>
